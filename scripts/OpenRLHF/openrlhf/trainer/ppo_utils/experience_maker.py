...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience_list(
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            elif self.advantage_estimator in ["reinforce", "rloo", "reinforce_baseline", "group_norm", "group_norm_diff", "group_norm_domain", "group_norm_wo_std"]:
    ...
    def process_experiences(self, experiences: List[Experience]) -> Tuple[List[Experience], List[torch.Tensor]]:
        ...
        if args.advantage_estimator == "rloo":
            ...
            rewards = rewards.reshape(-1).to(device="cpu").chunk(len(experiences))
            return experiences, rewards
        elif args.advantage_estimator == "group_norm_wo_std":
            rewards = torch.cat([experience.info["reward"] for experience in experiences])
            rewards = rewards.reshape(-1, args.n_samples_per_prompt).to(device="cuda")
            rewards = rewards - rewards.mean(-1, keepdim=True)
            rewards = rewards.reshape(-1).to(device="cpu").chunk(len(experiences))
            print(rewards)
            return experiences, rewards
        elif args.advantage_estimator == "group_norm_diff":
            rewards = torch.cat([experience.info["reward"] for experience in experiences])
            rewards = rewards.reshape(-1, args.n_samples_per_prompt).to(device="cuda")

            # Compute SC for each group (mean reward in the group)
            sc = rewards.mean(dim=-1, keepdim=True)  # shape: [batch_size, 1]
            gamma = 1.0
            diff_weight = torch.log1p(1 / (sc + 1e-5)).pow(gamma)  # shape: [batch_size, 1]
            # diff_weight = (1.0 / (sc + 1e-5)).pow(gamma)
            rewards = (rewards - rewards.mean(-1, keepdim=True)) / (rewards.std(-1, keepdim=True) + 1e-9)
            rewards = torch.where(rewards < 0, rewards * diff_weight, rewards)
            rewards = rewards.reshape(-1).to(device="cpu").chunk(len(experiences))
            return experiences, rewards
        elif args.advantage_estimator == "group_norm_domain":
            rewards = torch.cat([experience.info["reward"] for experience in experiences])
            rewards = rewards.reshape(-1, args.n_samples_per_prompt).to(device="cuda")

            # Step 1: Detect tail domain (if any reward > 1 in the group)
            is_tail = (rewards > 1.0).any(dim=-1, keepdim=True)  # shape: [batch_size, 1]
            tail_value = torch.tensor(2.57, device=rewards.device)
            head_value = torch.tensor(0.85, device=rewards.device)

            domain_weight = torch.where(
                is_tail,
                tail_value,
                head_value
            )  # shape: [batch_size, 1]
            # Step 2: Reset all reward values > 1 back to 1
            print(rewards)
            rewards.clamp_(max=1.0)
            print(rewards)
            # Step 3: Normalize rewards (GRPO-style)
            rewards_normalized = (rewards - rewards.mean(dim=-1, keepdim=True)) / (rewards.std(dim=-1, keepdim=True, unbiased=False) + 1e-9)
            # Step 4: Apply domain weight to negative advantages only
            print(rewards_normalized)
            rewards_weighted = torch.where(rewards_normalized < 0, rewards_normalized * domain_weight, rewards_normalized)
            print(rewards_weighted)
            # Step 5: Flatten and return
            rewards = rewards_weighted.reshape(-1).to(device="cpu").chunk(len(experiences))
            return experiences, rewards
        # default rewards
